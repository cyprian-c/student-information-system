...
class Fee
{
    ...
    public function assignFeeToStudent($studentId, $academicYear)
    {
        try {
            ...
            // Get fee structure for the student's class and academic year
            $feeStructure = $this->getFeeStructure($student['class'], $academicYear);

            if (!$feeStructure) {
                error_log("No fee structure found for class: " . $student['class'] . ", Year: " . $academicYear);
                return false;
            }
            ...
            if ($stmt->fetch()) {
                error_log("Fee already assigned for student ID: " . $studentId . ", Year: " . $academicYear);
                return true; // Already assigned - return true as it's not an error
            }

            // Calculate due date (30 days from enrollment or term start)
            $dueDate = date('Y-m-d', strtotime('+30 days'));

            // Assign fee to student
            $sql = "INSERT INTO student_fees 
                (student_id, academic_year, fee_structure_id, total_fee, amount_paid, due_date, status)
                VALUES 
                (:student_id, :academic_year, :fee_structure_id, :total_fee, 0.00, :due_date, 'pending')";
            ...
            if ($result) {
                error_log("Fee successfully assigned to student ID: " . $studentId .
                    ", Amount: KSh " . $feeStructure['total_fee'] .
                    ", Year: " . $academicYear);
            }

            return $result;
        } catch (PDOException $e) {
            error_log("Error assigning fee to student: " . $e->getMessage());
            return false;
        }
    }
